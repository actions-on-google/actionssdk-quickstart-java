...
public class MyActionsApp extends ActionsSdkApp {
  ...
  public ActionResponse handleDialog(ActionRequest request) throws IOException {
    Map<String, Object> conversationData = request.getConversationData();
    String query = request.getRawInput().getQuery().toLowerCase();
    if (query.equals("merchant transaction")) {
      conversationData.put("paymentType", "merchant_payment");
      return signIn(request);
    } else if (query.equals("google pay transaction")) {
      conversationData.put("paymentType", "google_payment");
      return checkTransactionRequirements(request);
    } else if (query.equals("check transaction requirements") || query.equals("check requirements")) {
      return checkTransactionRequirements(request);
    } else if (query.equals("get delivery address")) {
      return getDeliveryAddress(request);
    } else if (query.equals("confirm transaction")) {
      return sendTransactionDecision(request);
    } else if (query.equals("send order update")) {
      return sendOrderUpdate(request);
    } else {
      return getResponseBuilder(request)
        .add("I didn't get that.")
        .add("What would you like to try?")
        .build();
    }
  }

  private ActionResponse signIn(ActionRequest request) {
    // [START sign_in_asdk]
  ...
  private ActionResponse checkTransactionRequirements(ActionRequest request) {
  ...
  private ActionResponse getDeliveryAddress(ActionRequest request) {
  ...
  private ActionResponse sendTransactionDecision(ActionRequest request) {
    ...
    if (conversationData.get("paymentType") != null &&
      conversationData.get("paymentType").equals("google_payment")) {
  ...
  @ForIntent("actions.intent.TRANSACTION_DECISION")
  ...
  private ActionResponse sendOrderUpdate(ActionRequest request)
      throws IOException {
    Map<String, Object> conversationData = request.getConversationData();
    String orderId = (String) conversationData.get("latestOrderId");
    // [START order_update]
